...
`sha256(manufacturer + serialNumber + runId)` produces a unique, stable on-chain fingerprint
for each product unit. The manufacturer stores this hash on the Cardano blockchain together
with product details — name, production date, material info, and an image URL. Customers can
scan a QR code printed on the label to instantly verify the product's authenticity without
...
3. **Hash** — Computes `sha256(manufacturer + serialNumber + runId)` as the unique certificate
...
To verify programmatically, recompute the hash printed during issuance and call the
public API:
